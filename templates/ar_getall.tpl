...
<table width="100%" border="0" cellspacing="0" cellpadding="4" dir="rtl">
	<tr class="table_title"> 
		...
		?><td><div align="center"><?=$project['arr_fields_names'][$project['getall_fields'][$i]]['ar_friendly_name']?>
		<a href="{make_link controller="<?=$project['controller']?>" action="getall" __orderby=<?=$project['getall_fields'][$i]?> __desc="no"}"><img src="{$HTML_ROOT}view/images/up_arrow.png" border="0"/></a>
		<a href="{make_link controller="<?=$project['controller']?>" action="getall" __orderby=<?=$project['getall_fields'][$i]?> __desc="yes"}"><img src="{$HTML_ROOT}view/images/down_arrow.png" border="0"/></a>
		</div></td><?
		}
		?>
...
<div class="div_text" dir="rtl">الصفحة:
		{foreach key=k item=v from=$<?=$project['model']?>.__paging_pages}
			{if $v}
			<a href="{make_link controller="<?=$project['controller']?>" action="getall" __orderby=$__orderby __desc=$__desc array=$v}" class="mylink">{$k}</a>
			{else}
